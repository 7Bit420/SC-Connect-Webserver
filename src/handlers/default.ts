...
function handler(
    ...
    try {
        if (req.url == '/') {
            ...
            fs.existsSync(`${mainConfig.readir}/pages/${req.url}.html`) &&
            fs.lstatSync(`${mainConfig.readir}/pages/${req.url}.html`).isFile()
        ) {
            res.writeHead(200, 'OK', { 'Content-Type': 'text/html' })
            fs.createReadStream(`${mainConfig.readir}/pages/${req.url}.html`).pipe(res).on('drain', res.end)
        } else if (
            fs.existsSync(`${mainConfig.readir}/pages/${req.url}`) &&
            ...
            fs.createReadStream(`${mainConfig.readir}/pages/errors/404.html`).pipe(res).on('drain', res.end)
        }
    } catch (err) {
